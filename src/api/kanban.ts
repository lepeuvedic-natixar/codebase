...
export async function deleteColumn(columnId: string) {
  ...
  mutate(
    ...
    (currentBacklog: any) => {
      ...
      const columnsOrder = backlogs.columnsOrder.filter(
      ...
      if (storyId) {
        const currentStory = backlogs.userStory.filter(
          (story: KanbanUserStory) =>
            story.itemIds.filter((itemId) => itemId === newItem.id)[0],
        )[0]
...
export async function deleteItem(itemId: string | false) {
  ...
  mutate(
    ...
    (currentBacklog: any) => {
      ...
      const items = backlogs.items.filter(
        (item: KanbanItem) => item.id !== itemId,
      )
  ...
  mutate(
    ...
    (currentBacklog: any) => {
      ...
      backlogs.userStory.splice(
        backlogs.userStory.findIndex(
          (c: KanbanUserStory) => c.id === newStory.id,
        ),
        1,
        newStory,
      )
  ...
  mutate(
    ...
    (currentBacklog: any) => {
      ...
      const userStory = backlogs.userStory.filter(
        (column: KanbanUserStory) => column.id !== storyId,
      )
...
export function handlerKanbanDialog(selectedItem: string | boolean) {
  ...
  mutate(
    endpoints.key + endpoints.master,
    (currentKanbanMaster: any) => ({ ...currentKanbanMaster, selectedItem }),
    false,
  )
}
