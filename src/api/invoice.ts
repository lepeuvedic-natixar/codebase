...
export async function insertInvoice(newInvoice: InvoiceList) {
  ...
  mutate(
    ...
    (currentInvoice: any) => {
      ...
      const addedInvoice: InvoiceList[] = [
...
export async function updateInvoice(
  invoiceId: number,
  updatedInvoice: InvoiceList,
) {
  // to update local state based on key
  mutate(
    endpoints.key + endpoints.list,
    (currentInvoice: any) => {
      const newInvoice: InvoiceList[] = currentInvoice.invoice.map(
        (invoice: InvoiceList) =>
  ...
  mutate(
    ...
    (currentInvoice: any) => {
      const nonDeletedInvoice = currentInvoice.invoice.filter(
        (invoice: InvoiceList) => invoice.id !== invoiceId,
      )
...
export function handlerCustomerFrom(open: boolean) {
  ...
  mutate(
    endpoints.key + endpoints.actions,
    (currentInvoicemaster: any) => ({ ...currentInvoicemaster, open }),
    false,
  )
}
...
export function selectCountry(country: CountryType | null) {
  ...
  mutate(
    endpoints.key + endpoints.actions,
    (currentInvoicemaster: any) => ({ ...currentInvoicemaster, country }),
    false,
  )
}
...
export function handlerPreview(isOpen: boolean) {
  ...
  mutate(
    endpoints.key + endpoints.actions,
    (currentInvoicemaster: any) => ({ ...currentInvoicemaster, isOpen }),
    false,
  )
}
...
export function handlerDelete(alertPopup: boolean) {
  ...
  mutate(
    endpoints.key + endpoints.actions,
    (currentInvoicemaster: any) => ({ ...currentInvoicemaster, alertPopup }),
    false,
  )
}
